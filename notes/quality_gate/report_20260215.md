# Quality Gate Report - PilotSuite
...
|-------|---------------|-------------|
| Python Syntax | ✅ PASSED | ✅ PASSED |
| Unit Tests | ⚠️ BLOCKED | ❌ FAILED |
| Coverage | ⚠️ N/A | ⚠️ N/A |
| CHANGELOG Sync | ✅ PASSED | ✅ PASSED |
| Manifest Version | ✅ PASSED | ✅ PASSED |

**Overall Status:** ⚠️ **CONDITIONAL PASS** - Syntax OK, Tests Blocked
...
**Status:** ⚠️ **NOT DETERMINED**

Coverage analysis could not be completed due to test execution failures.

**Critical Modules Verified Present:**
- ✅ `neurons/` - 7 files (base.py, context.py, manager.py, mood.py, state.py, test_neurons.py)
- ✅ `mood/` - 6 files (actions.py, api.py, engine.py, orchestrator.py, scoring.py, service.py)
- ✅ `habitus_miner/` - 4 files (mining.py, model.py, service.py, store.py)
- ✅ `brain_graph/` - 8 files (api.py, bridge.py, feeding.py, model.py, provider.py, render.py, service.py, store.py)
...
```
✅ **Valid JSON, version present**

### Core Add-on
**File:** `addons/copilot_core/config.json`
```json
{
  "name": "PilotSuite Core (MVP)",
  "slug": "copilot_core",
  "version": "0.4.26",
  ...
}
