"""Contract tests for inventory_store.py copilot_ha → pilotsuite LEGACY_DOMAIN parity."""
...
SOURCE_FILE = f"{REPO_ROOT}/custom_components/pilotsuite/inventory_store.py"


class TestInventoryStoreProjection:
    """IS-owners: inventory_store.py LEGACY_DOMAIN parity."""

    def test_is1_kanonische_legacyj_domain_konstante(self):
        """IS1: Kanonische LEGACY_DOMAIN nutzt pilotsuite statt copilot_ha."""
    ...
    def test_is2_ast_scan_null_stale_copilot_ha_literal(self):
        """IS2: AST-Scan findet null stale copilot_ha-Literale in inventory_store.py."""
        tree = ast.parse(open(SOURCE_FILE).read())
        stale_nodes = [
        ...
        assert not stale_nodes, (
            f"Stale copilot_ha literals found in inventory_store.py: {stale_nodes}"
        )

    def test_is3_syntax_ok(self):
        """IS3: inventory_store.py kompiliert fehlerfrei."""
    ...
    def test_is4_legitimLegacy_store_key_format(self):
        """IS4: LEGACY_STORE_KEY nutzt LEGACY_DOMAIN-Variable korrekt formatiert."""
        tree = ast.parse(open(SOURCE_FILE).read())
        ...
        # LEGACY_STORE_KEY muss f"{LEGACY_DOMAIN}.inventory" sein
