"""Contract tests for scene_module.py — pilotsuite hass.data key parity."""
import ast
import pathlib
import sys

SRC = pathlib.Path("custom_components/pilotsuite/core/modules/scene_module.py")


def test_scene_module_key_is_pilotsuite():
    """SC1: SceneModule uses pilotsuite namespace in hass.data keys."""
    src = SRC.read_text()
...
def test_no_stale_copilot_ha_literals_in_scene_module():
    """SC2: No stale copilot_ha hass.data literals in scene_module.py."""
    src = SRC.read_text()
    tree = ast.parse(src)
    literals = [
...
def test_scene_module_syntax_ok():
    """SC3: scene_module.py parses cleanly."""
    src = SRC.read_text()
    try:
        ast.parse(src)
    except SyntaxError as e:
        raise AssertionError(f"scene_module.py has syntax error at line {e.lineno}: {e.msg}")
    # Also verify the module can be loaded as spec
    sys.path.insert(0, str(pathlib.Path.cwd()))
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location("scene_module", SRC)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
    except Exception as e:
        raise AssertionError(f"scene_module.py cannot be loaded: {e}")
    finally:
        sys.path.pop(0)


if __name__ == "__main__":
    test_scene_module_key_is_pilotsuite()
    test_no_stale_copilot_ha_literals_in_scene_module()
    test_scene_module_syntax_ok()
