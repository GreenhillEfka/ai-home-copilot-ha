"""Contract tests for birthday_reminder_module.py — pilotsuite hass.data key parity."""
import ast
import pathlib
import sys

SRC = pathlib.Path("custom_components/pilotsuite/core/modules/birthday_reminder_module.py")


def test_birthday_reminder_key_is_pilotsuite():
    """BR1: BirthdayReminderModule uses pilotsuite namespace in hass.data keys."""
    src = SRC.read_text()
...
def test_no_stale_copilot_ha_literals_in_birthday_reminder():
    """BR2: No stale copilot_ha hass.data literals in birthday_reminder_module.py."""
    src = SRC.read_text()
    tree = ast.parse(src)
    literals = [
...
def test_birthday_reminder_syntax_ok():
    """BR3: birthday_reminder_module.py parses cleanly."""
    src = SRC.read_text()
    try:
        ast.parse(src)
    except SyntaxError as e:
        raise AssertionError(f"birthday_reminder_module.py has syntax error at line {e.lineno}: {e.msg}")
    # Also verify the module can be loaded as spec
    sys.path.insert(0, str(pathlib.Path.cwd()))
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location("birthday_reminder_module", SRC)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
    except Exception as e:
        raise AssertionError(f"birthday_reminder_module.py cannot be loaded: {e}")
    finally:
        sys.path.pop(0)
