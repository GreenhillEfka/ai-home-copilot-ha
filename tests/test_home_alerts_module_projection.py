"""Contract tests for home_alerts_module.py STORAGE_KEY projection."""
...
def test_HA_module_1_canonical_storage_key() -> None:
    ...
    print(f"HA-module-1 passed: STORAGE_KEY = {collector.keys}")


def test_HA_module_2_no_stale_copilot_ha() -> None:
...
def test_HA_module_3_syntax() -> None:
    """HA-module-3: File has valid Python syntax."""
    try:
        ast.parse(CONTENT)
    except SyntaxError as exc:
        raise AssertionError(f"Syntax error in home_alerts_module.py: {exc}") from exc
    print("HA-module-3 passed: syntax OK")


if __name__ == "__main__":
    test_HA_module_3_syntax()
    test_HA_module_1_canonical_storage_key()
    test_HA_module_2_no_stale_copilot_ha()
