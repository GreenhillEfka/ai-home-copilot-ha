"""Tests for RAG Hybrid Search API.

Comprehensive test suite for BM25, hybrid search, and API endpoints.
"""
...
class TestEdgeCases:
    ...
    def test_bm25_special_characters_in_query(self, index_factory):
    ...
    def test_bm25_very_long_document(self, index_factory):
    ...
    def test_bm25_unicode_text(self, index_factory):
...
@pytest.fixture
def index_factory():
    """Factory for creating BM25 indexes."""
    indexes = []
    ...
    def create_index():
        ...
        config = BM25Config(db_path=db_path, persist=True)
        idx = BM25SqliteIndex(config)
        indexes.append((idx, db_path))
        return idx
    
    yield create_index
    
    for idx, db_path in indexes:
        idx.close()
        if os.path.exists(db_path):
            os.unlink(db_path)
